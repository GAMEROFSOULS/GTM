...
import { Download, ExternalLink, RefreshCcw, Search, Loader2 } from "lucide-react";
...
function toCSV(leads: Lead[]): string {
  const headers = ["Name", "Email", "LinkedIn", "Company", "Score", "Tier", "Reason", "Side", "Date", "Status"];
  const escape = (v: unknown) => {
    const s = String(v ?? "");
    return `"${s.replace(/"/g, '""')}"`;
  };
  const rows = leads.map(l =>
    [l.name, l.email, l.linkedin, l.company, l.score, l.tier, l.reason, l.side, l.qualified_at, l.status]
      .map(escape).join(",")
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            All qualified leads from your GTM agent.{" "}
            {!isLoading && !isError && (
              <span className="text-primary font-medium">{filtered.length} lead{filtered.length !== 1 ? "s" : ""}</span>
            )}
          </p>
        </div>
        <Button
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2 min-w-[72px]">
                          <span className="font-mono text-sm w-4">{l.score}</span>
